# Loading packages here which we need
import sys
import argparse
import numpy as np
...
def graph_centrality(df, cent_type='betweenness', keep_thresh=0.5, cond_type='add_one', corr_type='spearman', weighted=False, corr_dir='none', min_connected=0):
    ...
    if corr_dir == 'positive':
        ...
        w_corr_df_b = 1 - abs(w_corr_df.copy()) # keep both strong positive and negative correlations ( ##Umair This dsiscards negative correlation)
    ...
    temp = abs(w_corr_df_b.copy()) # This discards neagtive correlation in reality ## Umair's Remarks
...
def plot_graph_centrality(features, cond_type, corr_type, corr_dir, keep_thresh, weighted):
    ...
    if corr_dir == 'positive':
        ...
        ################################### To keep negative correlation ################################
        w_corr_df_b = w_corr_df.copy() # keeps negative correlation by removing abs statement
        ###############################################################################################
    ...
    ####################################Addition to take -ve Correlation into account##################
    #temp = w_corr_df_b.copy() # To take negative correlation into account
    ###################################################################################################
    ...
    ####################################Addition to take -ve Correlation into account##################
    #df_b = df_b.loc[((df_b[attr] <= 1 - keep_thresh)),:] # To take negative correlation into account
    ###################################################################################################
...
def main(ab_comp, infile1, infile2, metric_name, c_type, min_count,
    ...
    global outdir
    outdir = 'Chiquet_Results/'+infile1_path.split('/')[-1].split('.')[0]+'/' +metric_name +'_' + correlation +'_' + str(keep_threshold) +'_'+ centrality_type
